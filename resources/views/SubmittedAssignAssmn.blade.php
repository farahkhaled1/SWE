...
<head>
    ...
    <title>SUBMITTED assignment assessment1</title>
    ...
    <meta property="og:title" content="assignment assessment1" />
...
<body class="u-body u-white u-xl-mode">
    <header class="u-header u-section-row-container" id="sec-f4f6" data-animation-name="" data-animation-duration="0"
        ...
        <div class="u-section-rows">
            ...
            <div class="u-custom-color-1 u-section-row u-section-row-3" id="sec-0c3e">
                ...
                ">
                            <a class="u-button-style u-custom-active-border-color u-custom-border u-custom-border-color u-custom-borders u-custom-hover-border-color u-custom-padding-bottom u-custom-text-active-color u-custom-text-color u-custom-text-hover-color u-custom-top-bottom-menu-spacing u-nav-link"
                                ...
                                <svg class="u-svg-link" viewBox="0 0 24 24">
                                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-b5a4"></use>
                                </svg>
                                <svg class="u-svg-content" version="1.1" id="svg-b5a4" viewBox="0 0 16 16" x="0px"
                                    y="0px" xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <g>
                                        <rect y="1" width="16" height="2"></rect>
    ...
    <section class="u-align-center u-clearfix u-section-1" id="sec-a9b1" style="min-height: 100vh">
        <div class="u-clearfix u-sheet u-sheet-1">
                    ...
                    <tbody class="u-table-body">
                        <tr style="height: 64px">
                            ...
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div >  @php
                                 $data=DB::Table('assignassessment')->select('ilo')->where('prof_id',Auth::user()->id)->get();
                            ...
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div >    @php
                                     $data=DB::Table('assignassessment')->select('assignNum')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->assignNum ;}
                                            @endphp
                                </div>
                            </td>
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div > @php
                                     $data=DB::Table('assignassessment')->select('assignTitle')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->assignTitle ;}
                                            @endphp
                                          </div>
                            </td>
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div >@php
                                     $data=DB::Table('assignassessment')->select('AssessmentCriteria')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->AssessmentCriteria ;}
                                            @endphp
                                            </div>
                            </td>
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div >@php
                                     $data=DB::Table('assignassessment')->select('gradesWeight')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->gradesWeight ;}
                                            @endphp
                                            </div>
                            </td>
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div >@php
                                     $data=DB::Table('assignassessment')->select('issueDate')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->issueDate ;}
                                            @endphp
                                            </div>
                            </td>
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div >@php
                                     $data=DB::Table('assignassessment')->select('DueDate')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->DueDate ;}
                                            @endphp
                                            </div>
                            ...
                            <td class="u-border-1 u-border-grey-30 u-table-cell">
                                <div>@php
                                     $data=DB::Table('assignassessment')->select('comments')->where('prof_id',Auth::user()->id)->get();
                                            foreach ($data as $key => $value) {
                                            echo $value->comments ;}
                                            @endphp
                                           </div>
...
<!-- 
            ...
            <h4 class="u-text u-text-default u-text-1">Date:</h4> -->
        </div>
